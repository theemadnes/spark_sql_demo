...
tshirt_order_file_lines = tshirt_order_file_source_file.map(lambda x: x.split(','))
tshirt_order_file_lines_inferred_types_rows = tshirt_order_file_lines.map(lambda x: Row(full_name = x[0], shirt_size  = x[1], us_state = x[2], shirt_quantity = int(x[3])))
state_population_lines = state_population_source_file.map(lambda x: x.split(','))
state_population_lines_inferred_types_rows = state_population_lines.map(lambda x: Row(us_state = x[0], state_population = x[1]))
...
orders_over_100 = sql_context.sql("SELECT * FROM t_shirt_orders WHERE t_shirt_orders.shirt_quantity >= 100")
